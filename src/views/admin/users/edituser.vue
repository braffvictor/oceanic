<template>
  <main class="min-h-screen bg-slate-100 dark:bg-slate-900 transit">
    <section class="md:w-7/12 mx-auto w-11/12 pt-5">
      ...
      >
        <div class="flex justify-between items-center">
          ...
          <div class="text-right float-left relative transit mt-1">
            <svg-comp
              ...
              Sclass="mx-auto text-center !stroke-slate-100 dark:!stroke-slate-900 cursor-pointer"
            />
            <!-- options for nft menu -->
            <div
              class="absolute shadow-md h-auto -left-44 md:left-5 transit top-5 md:top-3 md:text-base text-sm md:text- font-normal z-20 w-52 rounded-xl bg-slate-100 dark:bg-slate-800 flex flex-col divide-y p-1 scale-y-0 peer-hover:scale-y-100 -translate-y-full divide-slate-900 dark:divide-slate-100 divide-opacity-20 dark:divide-opacity-45 peer-hover:-translate-y-0 md:translate-y-0 opacity-0 peer-hover:opacity-100 gap-y-2 cursor-pointer peer-hover:block py-2 hover:scale-100 hover:-translate-y-0 hover:block md:hover:opacity-100 text-start"
            >
              <p
                class="p-1 py-3 px-2 active:bg-slate-200 break-words hover:bg-slate-200 dark:text-slate-100 dark:hover:bg-slate-700 text-slate-900 dark:active:bg-slate-700"
                @click="
                  $router.push(
    ...
    <DDialog
      ...
      :data="type"
      :screen="true"
      :title="`Deposit into or Withdraw from ${
        user && user.fullName
      } Wallet Balance`"
    />
  </main>
