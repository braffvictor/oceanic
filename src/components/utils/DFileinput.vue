<template>
  <div>
    ...
    <section class="pb-4">
      <label
        :for="name"
        class="group mt-2 p-1 dark:p-2 dark:text-slate-100 text-slate-900 overflow-hidden caret:slate-700 dark:caret-slate-100 h-[55px] has-[:focus]:border-green-500 outline-none transit relative z-0 flex items-center gap-x-3"
        :class="[
          !type || type == 'default'
          ...
          type == 'outlined'
            ...
            : '',
          type == 'filled'
            ? 'bg-slate-200 dark:bg-slate-900 rounded-tl-lg rounded-tr-lg border-b dark:border-b-gray-600  hover:border-b-green-500 border-b-gray-400 dark:hover:bg-slate-800 hover:bg-slate-200 p-2'
            : '',
        ]"
      >
        <div v-if="icon">
