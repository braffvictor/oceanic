<template>
  ...
  >
    <!-- navbar section -->
    <nav
      class="p-4 md:px-8 bg-red flex items-center justify-between border-b fixed w-full backdrop-blur-sm"
      :class="
    ...
    >
      ...
      <div class="inline-flex gap-x-3">
        <button
          v-for="text in ['Collections', 'Explore', 'About', 'Contact']"
          :key="text"
          class="p-2 px-3 dark:text-slate-200 dark:active:bg-green-100 dark:hover:text-green-400 hover:text-green-500 transit rounded-lg active:bg-green-200 md:block hidden text-slate-900"
        >
          {{ text }}
        </button>
    ...
    <div
      class="w-[100vw] fixed top-0 border-b transition-all ease-in-out duration-1000 !overflow-hidden backdrop-blur-md"
      :class="[
        show ? 'md:h-[58%] h-[50%]' : 'h-0',
        themeState == 'light' || themeState == null
          ? 'whiteT border-b-gray-200'
          : 'darkT border-b-gray-600',
      ]"
    >
      ...
      <div class="flex mt-6 ml-10 gap-x-2">
        <d-button
          type="outlined"
          class="border-green-500 px-4 text-green-500 dark:text-green-500"
          >Login</d-button
        >
        <d-button
          type="elevated"
